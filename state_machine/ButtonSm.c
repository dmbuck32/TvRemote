...
static void NOT_PRESSED_enter(ButtonSm* sm)
{
    ...
    // uml: enter / { reset_timer(); }
    {
        ...
        sm->vars.timer_started_at_ms = timeInMilliseconds();
...
static void NOT_PRESSED_do(ButtonSm* sm)
{
  ...
  && ((timeInMilliseconds() - sm->vars.timer_started_at_ms) >= DEBOUNCE_TIMEOUT))
    {
...
static void PRESSED_do(ButtonSm* sm)
{
    ...
    if ((!sm->vars.input_is_pressed) && ((timeInMilliseconds() - sm->vars.timer_started_at_ms) >= DEBOUNCE_TIMEOUT))
    {
        // Step 1: Exit states until we reach `ROOT` state (Least Common Ancestor for transition).
